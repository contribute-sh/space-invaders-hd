...
## Architecture
- `src/main.ts` owns bootstrap and the fixed-timestep loop. It creates the initial `GameState`, polls the keyboard adapter each frame, advances the simulation at 60 Hz, renders the latest state, and derives audio triggers by comparing the previous and next states.
- `src/game/state.ts` defines the core types, constants, and initial-state factory for the playfield, player, invaders, projectiles, score, lives, and phase flags.
- `src/game/step.ts` is the pure simulation boundary. It applies input, movement, cooldowns, invader marching and descent, collision resolution, score/life updates, pause handling, wave-clear, and game-over transitions without touching DOM, audio, or timers outside the state object.
- `src/input/keyboard.ts` converts browser key events into a typed per-frame `Input` snapshot with edge-triggered actions for fire, pause, and start/continue.
...
- `src/audio/sfx.ts` exposes a tiny sound facade backed by `AudioContext`, oscillator nodes, and gain envelopes. Audio remains optional at runtime: if the context cannot start, gameplay continues silently.
...
Primary action: `Enter` starts a new run.
...
- Loading: a short bootstrap label such as `Initializing canvas/audio...` appears inside the overlay until the first frame is ready.
- Error: if Canvas 2D is unavailable, replace the canvas area with a blocking message that the browser is unsupported; if audio setup fails, show a non-blocking `Sound unavailable` note and continue.
...
- Life-loss behavior: if the invader formation reaches the player collision/fail line, the game freezes briefly, subtracts one life, and resets the current wave to its starting formation while preserving score and remaining lives; if no lives remain, it transitions to game over instead.
...
Primary action: `Enter` begins the next wave.

Layout: the HUD remains visible while a centered overlay announces `Wave Clear`, shows the current score and remaining lives, and prompts `Press Enter for Next Wave`.
Empty / error / loading:
...
Primary action: `Enter` starts a fresh run from wave 1.

Layout: the final playfield freezes under a centered `Game Over` card that displays final score, wave reached, and `Press Enter to Restart`.
Empty / error / loading:
...
- `/` Playing -> Game over -> Start-equivalent restart on `Enter`
...
```ts
type GamePhase = "start" | "playing" | "paused" | "waveClear" | "gameOver";
...
- Use a flat `invaders: Invader[]` list with `row` and `col` metadata rather than a nested matrix. This simplifies collision filtering, rendering iteration, and speed-up rules as invaders are removed.
...
1. Scaffold the toolchain and page shell.
   Add `package.json`, `tsconfig.json`, `eslint.config.js`, `vitest.config.ts`, and `index.html` so the repo can lint, typecheck, test, build, and mount a single canvas entrypoint with minimal inline CSS.

2. Define the simulation model.
   Implement `src/game/state.ts` with the typed arena, player, invader, projectile, HUD, and phase models, plus constants and an initial-state factory for the 5x11 formation and starting lives/score.

3. Implement the pure `step()` loop.
   Add movement clamping, shooting cooldown, projectile advancement and cleanup, invader marching and descending, bullet-vs-invader collision, invader reach/player collision handling, score updates, life loss with current-wave reset, pause toggling, wave clear, and game-over transitions.
...
5. Add the input adapter and renderer.
   Implement `src/input/keyboard.ts` for stable frame snapshots and `src/render/canvas.ts` for drawing the playfield, HUD, and overlay states with procedural geometry sized against the logical arena.

6. Wire audio and the runtime loop.
   Finish `src/main.ts` with the `requestAnimationFrame` accumulator, fixed 60 Hz stepping, render calls, start/restart flow, and transition-based SFX playback through `src/audio/sfx.ts` with graceful mute fallback if audio cannot start.
...
## Acceptance criteria
- `pnpm install --frozen-lockfile`, `pnpm eslint .`, `pnpm tsc --noEmit`, `pnpm vitest run`, and `pnpm build` all succeed.
- Opening the built or previewed app shows a single centered canvas game shell on route `/`.
- The start overlay explains controls and `Enter` starts a new run.
- Left and right arrow keys move the player ship and clamp it within the playfield bounds.
- Pressing `Space` fires player shots with a cooldown; shots travel upward and are removed when they leave the arena or hit an invader.
- A 5x11 invader formation marches horizontally, descends when it hits an edge, and speeds up as invaders are removed.
- Player shots destroy invaders, increase score, and trigger a hit SFX.
- If the invader formation reaches the player collision/fail line, the run loses one life, plays the player-death SFX, and resets the current wave to its initial 5x11 layout while preserving score; if lives reach zero, the game transitions to the game-over overlay.
- Clearing all invaders shows a wave-clear overlay and allows the player to continue to the next wave with `Enter`.
- Losing the final life shows a game-over overlay with final score and restart prompt.
